...
{
    gf2d_draw_rect(gf2d_rect_to_sdl_rect(r),vector4d(color.r,color.g,color.b,color.a));
}
...
void gf2d_shape_draw(Shape shape,Color color,Vector2D offset)
{
    switch(shape.type)
    {
        case ST_RECT:
            ...
            break;
        case ST_CIRCLE:
            gf2d_draw_circle(vector2d(shape.s.c.x + offset.x,shape.s.c.y + offset.y), shape.s.c.r,vector4d(color.r,color.g,color.b,color.a));
            break;
        case ST_EDGE:
            gf2d_draw_line(vector2d(shape.s.e.x1 + offset.x,shape.s.e.y1 + offset.y),vector2d(shape.s.e.x2 + offset.x,shape.s.e.y2 + offset.y), vector4d(color.r,color.g,color.b,color.a));
            break;
    }
}
